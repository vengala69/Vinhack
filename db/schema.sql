...
CREATE INDEX sleep_logs_student_date_idx ON sleep_logs (student_id, sleep_date DESC);
...
CREATE INDEX screen_time_student_date_idx ON screen_time (student_id, date DESC);
...
CREATE INDEX academic_tasks_student_status_idx ON academic_tasks (student_id, status);
CREATE INDEX academic_tasks_student_due_idx    ON academic_tasks (student_id, due_date);
...
CREATE INDEX calendar_events_student_start_idx ON calendar_events (student_id, start_time);
...
CREATE INDEX mood_energy_student_date_idx ON mood_energy (student_id, date DESC);
...
CREATE INDEX study_sessions_student_start_idx ON study_sessions (student_id, start_time DESC);
...
CREATE INDEX daily_metrics_student_date_idx ON daily_metrics (student_id, date DESC);
...
CREATE TRIGGER sleep_logs_duration_ins AFTER INSERT ON sleep_logs
...
CREATE TRIGGER study_sessions_duration_ins AFTER INSERT ON study_sessions
...
CREATE TRIGGER academic_tasks_completed_ins AFTER INSERT ON academic_tasks
WHEN NEW.status = 'completed' AND NEW.completed_at IS NULL
BEGIN
    UPDATE academic_tasks SET completed_at = datetime('now') WHERE task_id = NEW.task_id;
END;

CREATE TRIGGER academic_tasks_completed_upd AFTER UPDATE OF status ON academic_tasks
WHEN (NEW.status = 'completed' AND NEW.completed_at IS NULL)
  OR (NEW.status <> 'completed' AND NEW.completed_at IS NOT NULL)
BEGIN
    UPDATE academic_tasks
       SET completed_at = CASE WHEN NEW.status = 'completed' THEN datetime('now') ELSE NULL END
...
SELECT t.*,
       (julianday(t.due_date) - julianday('now')) * 24.0 AS hours_until_due,
...
CREATE VIEW v_committed_by_day AS
SELECT student_id,
       date(start_time)                                          AS date,
